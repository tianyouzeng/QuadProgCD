function [c_sol,x_sol,I,tab]=simplex_with_Bland_pivoting_rule(tab,I,m,n,max_nb_iters,debug)
...
while keep_running&& nb_iters<max_nb_iters
    nb_iters=nb_iters+1;
    if any(tab(1,1:n)>0) %check if there is negative cost coeff.
        J1=find(tab(1,1:n)>0);
        pivot_column=J1(1);%(Bland's pivoting rule)
        %[~,pivot_column] = max(tab(1,1:n)); %yes, find the most negative 
...
if ~(min(tab(1,I))==0&&max(tab(1,I))==0)
